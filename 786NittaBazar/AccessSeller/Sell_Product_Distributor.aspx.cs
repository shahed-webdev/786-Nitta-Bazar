    ...
    public partial class Sell_Product_Distributor : System.Web.UI.Page
    {
        ...
        public static string GetCustomers(string prefix)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DBConnectionString"].ConnectionString))
            {
                ...
                {
                    cmd.CommandText = "SELECT top(2) Registration.UserName, Registration.Name, Registration.Phone,Seller.IsCIP, Seller.SellerID FROM Registration INNER JOIN Seller ON Registration.RegistrationID = Seller.SellerRegistrationID WHERE Seller.SellerID <> @SellerID AND Registration.UserName LIKE @UserName + '%'";
        ...
        public static string GetProduct(string prefix, string Client_SellerID)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DBConnectionString"].ConnectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "SELECT TOP(2) Product_Point_Code.Product_Code, Product_Point_Code.Product_Name, Product_Point_Code.Product_Price, Product_Point_Code.Product_Point, Product_Point_Code.Product_PointID, Seller_Product.SellerProduct_Stock,(Product_Point_Code.Seller_Commission * Seller.CommissionPercentage / 100) AS Seller_Commission FROM Product_Point_Code INNER JOIN Seller_Product ON Product_Point_Code.Product_PointID = Seller_Product.Product_PointID CROSS JOIN Seller WHERE (Seller_Product.SellerID = @SellerID) AND (Seller.SellerID = @Client_SellerID) AND ((Seller.IsCIP = 0 AND Product_Point_Code.IsCIP = 0) OR (Seller.IsCIP = 1)) AND (Seller_Product.SellerProduct_Stock > 0) AND (Product_Point_Code.Product_Code LIKE @Product_Code + '%')";
