<%@ Page Title="Order Product" Language="C#" MasterPageFile="~/Seller.Master" AutoEventWireup="true" CodeBehind="Order_Product.aspx.cs" Inherits="NittaBazar.AccessSeller.Order_Product" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    ...
    <asp:FormView ID="SellerFormView" runat="server" DataSourceID="SellerSQL" Width="100%">
        <ItemTemplate>
            ...
            <small class="alert-success">Balance:
                <label>৳<%#Eval("Available_LoadBalance","{0:N}") %></label>
                </small></h3>
        </ItemTemplate>
    </asp:FormView>

    <asp:SqlDataSource ID="SellerSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="SELECT Seller.Available_LoadBalance FROM Seller INNER JOIN Registration ON Seller.SellerRegistrationID = Registration.RegistrationID WHERE (Registration.RegistrationID = @RegistrationID)">
        <SelectParameters>
    ...
    <div class="table-responsive">
        <asp:GridView ID="ProductGridView" runat="server" AutoGenerateColumns="False" CssClass="mGrid table-hover" DataKeyNames="Product_PointID" DataSourceID="Product_PointSQL" AllowSorting="True">
            <Columns>
                <asp:TemplateField HeaderText="Add">
                    <ItemTemplate>
                        <button data-id="<%#Eval("Product_PointID") %>" data-name="<%# Eval("Product_Name") %>" data-price="<%#Eval("Product_Price") %>" data-point="<%# Eval("Product_Point") %>" data-stock="<%# Eval("Stock_Quantity") %>" data-commission="<%# Eval("Seller_Commission") %>" data-quantity="1" data-image="/CSS/Image/Cart.png" type="button" class="my-cart-btn"><i class="fas fa-cart-plus"></i></button>
                    </ItemTemplate>
        ...
        <asp:SqlDataSource ID="Product_PointSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="SELECT Product_Point_Code.Product_PointID, Product_Point_Code.IsCIP,Product_Point_Code.Product_Name, Product_Point_Code.Product_Code, Product_Point_Code.Product_Point, Product_Point_Code.Product_Price, Product_Point_Code.Stock_Quantity, Product_Point_Code.Order_Quantity, Product_Point_Code.Net_Quantity, Product_Point_Code.Seller_Commission * Seller.CommissionPercentage / 100 AS Seller_Commission FROM Product_Point_Code CROSS JOIN Seller WHERE (Product_Point_Code.Stock_Quantity &gt; 0) AND (Product_Point_Code.IsActive = 1) AND (Seller.SellerID = @SellerID) AND (Product_Point_Code.IsCIP = @IsCIP) ORDER BY Product_Point_Code.Product_Code" FilterExpression="Product_Name LIKE '{0}%' OR Product_Code LIKE '{0}%'" CancelSelectOnNullParameter="False">
            <FilterParameters>
            ...
            <SelectParameters>
                ...
                <asp:SessionParameter Name="IsCIP" DbType="Boolean" SessionField="CIP" />
            </SelectParameters>
        </asp:SqlDataSource>
    </div>
