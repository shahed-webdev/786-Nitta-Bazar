<%@ Page Title="Profile" Language="C#" MasterPageFile="~/Seller.Master" AutoEventWireup="true" CodeBehind="Seller_Profile.aspx.cs" Inherits="NittaBazar.AccessSeller.Seller_Profile" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <link href="CSS/Seller_Profile.css?v=3.3" rel="stylesheet" />
    <style>
        .cip { color: #f3b300; }
        .Main-balance { background-color: #00ff90; padding: 10px; font-size: 16px; border-radius: 10px; margin-bottom: 15px; }
    </style>
</asp:Content>
    ...
    <asp:FormView ID="SellerFormView" runat="server" DataKeyNames="RegistrationID" DataSourceID="SellerSQL" OnItemUpdated="SellerFormView_ItemUpdated" Width="100%">
        ...
        <ItemTemplate>
            ...
            <div class="Main-balance">
                Available Balance:
                   <strong>৳<%#Eval("Available_Balance","{0:N}") %></strong>
            </div>
        </ItemTemplate>
    </asp:FormView>
                    ...
                    <div class="row text-center">
                        <div class="col-md-3 col-sm-6">
                            <div class="SellingPoint Box">
                                <h5>Selling Income: ৳<%# Eval("Selling_Income") %></h5>
                                <h5>Selling Point: <%# Eval("SellingPoint") %></h5>
                            </div>
                        </div>
                        <div class="col-md-3 col-sm-6">
                            <div class="Selling_Income Box">
                                <a href="Commission_Records.aspx" title="Click to details">
                                    <h5>Total Income: ৳<%#Eval("Total_Income","{0:N}") %> <i class="fa fa-angle-right"></i></h5>
                                </a>
                                <% if (Session["CIP"].ToString() == "True")
                                    { %>
                                <a href="CIP_Commission_Records.aspx" title="Click to details">
                                    <h5>CIP Commission: ৳<%#Eval("CIP_Income","{0:N}") %> <i class="fa fa-angle-right"></i></h5>
                                </a>
                                <%}
                                    else
                                    { %>
                                <h5 style="padding: 10px 0"></h5>
                                <%} %>
                            </div>
                        </div>
                        <div class="col-md-3 col-sm-6">
                            <div class="Withdraw Box">
                                <a href="Withdraw_Details.aspx" title="Click to details">
                                    <h5>Withdraw: ৳<%#Eval("Withdraw_Balance","{0:N}") %> <i class="fa fa-angle-right"></i></h5>
                                </a>
                                <a href="Received_Balance.aspx" title="Click to details">
                                    <h5>Received: ৳<%#Eval("Received__Balance","{0:N}") %> <i class="fa fa-angle-right"></i></h5>
                                </a>
                            </div>
                        </div>
                        <div class="col-md-3 col-sm-6">
                            <div class="Balance Box">
                                <h5>Buying Amount: ৳<%# Eval("Buying_Amount") %></h5>
                                <h5>Av.Load Balance: ৳<%#Eval("Available_LoadBalance","{0:N}") %></h5>
                            </div>
                        </div>
                    </div>
                </div>
