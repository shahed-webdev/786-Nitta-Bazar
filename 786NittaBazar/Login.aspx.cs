...
namespace NittaBazar
{
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void CustomerLogin_LoggedIn(object sender, EventArgs e)
        {
            ...
            if (Roles.IsUserInRole(CustomerLogin.UserName.Trim(), "Authority")) //for Authority
            {
            ...
            if (Roles.IsUserInRole(CustomerLogin.UserName.Trim(), "Admin")) //for Admin
            {
                SqlCommand InstitutionID_cmd = new SqlCommand("SELECT InstitutionID from Registration where UserName = @UserName", con);
                InstitutionID_cmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());
            ...
            if (Roles.IsUserInRole(CustomerLogin.UserName.Trim(), "Sub-Admin")) //for Sub-Admin
            {
                SqlCommand InstitutionID_cmd = new SqlCommand("SELECT InstitutionID from Registration where UserName = @UserName", con);
                InstitutionID_cmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());
            ...
            if (Roles.IsUserInRole(CustomerLogin.UserName.Trim(), "Seller")) //for Seller
            {
                SqlCommand InstitutionID_cmd = new SqlCommand("SELECT InstitutionID from Registration where UserName = @UserName", con);
                InstitutionID_cmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());
                ...
                SellerIDcmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());

                SqlCommand CIPcmd = new SqlCommand("SELECT Seller.IsCIP FROM Registration INNER JOIN Seller ON Registration.RegistrationID = Seller.SellerRegistrationID WHERE (Registration.UserName = @UserName)", con);
                CIPcmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());

                con.Open();
                Session["InstitutionID"] = InstitutionID_cmd.ExecuteScalar().ToString();
                Session["RegistrationID"] = RegistrationIDcmd.ExecuteScalar().ToString();
                Session["SellerID"] = SellerIDcmd.ExecuteScalar().ToString();
                Session["CIP"] = CIPcmd.ExecuteScalar().ToString();
                con.Close();
            }

            if (Roles.IsUserInRole(CustomerLogin.UserName.Trim(), "Member")) //for Member
            {
                SqlCommand InstitutionID_cmd = new SqlCommand("SELECT InstitutionID from Registration where UserName = @UserName", con);
                InstitutionID_cmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());
                ...
                SqlCommand MemberInfo_cmd = new SqlCommand("SELECT Member.IsCIP,Member.Is_Identified,Member.MemberID FROM Member INNER JOIN Registration ON Member.MemberRegistrationID = Registration.RegistrationID WHERE (Registration.UserName = @UserName)", con);
                MemberInfo_cmd.Parameters.AddWithValue("@UserName", CustomerLogin.UserName.Trim());
                ...
                while (Member.Read())
                {
                    ...
                    Session["CIP"] = Member["IsCIP"].ToString();
                }
                con.Close();
            }
        }
