<%@ Page Title="Product Selling Report" Language="C#" MasterPageFile="~/Basic.Master" AutoEventWireup="true" CodeBehind="Product_Selling_Report.aspx.cs" Inherits="NittaBazar.AccessAdmin.Accounts.Product_Selling_Report" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    ...
    <div class="form-inline NoPrint">
        <div class="form-group">
            <asp:DropDownList ID="Seller_DropDownList" runat="server" CssClass="form-control" DataSourceID="SellerSQL" DataTextField="Name" DataValueField="SellerID" AppendDataBoundItems="True">
                <asp:ListItem Value="%">All Distributor</asp:ListItem>
            </asp:DropDownList>
            <asp:SqlDataSource ID="SellerSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="SELECT Registration.Name + ' (' + Registration.UserName + ')' AS Name, Seller.SellerID FROM Seller INNER JOIN Registration ON Seller.SellerRegistrationID = Registration.RegistrationID"></asp:SqlDataSource>
        </div>
        <div class="form-group">
    ...
    <asp:SqlDataSource ID="TotalSQL" runat="server" CancelSelectOnNullParameter="False" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="SELECT SUM(Shopping.ShoppingAmount) AS Total FROM Shopping INNER JOIN Member ON Shopping.MemberID = Member.MemberID INNER JOIN Registration ON Member.MemberRegistrationID = Registration.RegistrationID WHERE (CAST(Shopping.ShoppingDate AS DATE) BETWEEN ISNULL(@From_Date, N'1-1-1000') AND ISNULL(@To_Date, N'1-1-3000')) AND (CAST(Shopping.SellerID AS nvarchar(50)) LIKE @SellerID) AND (Registration.UserName LIKE @UserName)">
        <SelectParameters>
            ...
            <asp:ControlParameter ControlID="Seller_DropDownList" Name="SellerID" PropertyName="SelectedValue" />
