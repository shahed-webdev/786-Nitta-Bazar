...
namespace NittaBazar.AccessAdmin
{
    public partial class Add_Product_Point : System.Web.UI.Page
    {
        ...
        protected void UpdateCIPCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            CheckBox cb = (CheckBox)sender;
            GridViewRow row = (GridViewRow)cb.NamingContainer;
            var U_point_Label = row.FindControl("U_point_Label") as Label;
            var p = Convert.ToDouble(U_point_Label.Text);

            if (p >= 500)
            {
                CIPupdateSQL.UpdateParameters["IsCIP"].DefaultValue = cb.Checked.ToString();
                CIPupdateSQL.UpdateParameters["Product_PointID"].DefaultValue = ProductGridView.DataKeys[row.RowIndex]["Product_PointID"].ToString();
                CIPupdateSQL.Update();
            }
            else
            {
