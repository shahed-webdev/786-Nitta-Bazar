<%@ Page Title="Add Product" Language="C#" MasterPageFile="~/Basic.Master" AutoEventWireup="true" CodeBehind="Add_Product_Point.aspx.cs" Inherits="NittaBazar.AccessAdmin.Add_Product_Point" %>
        ...
        <ContentTemplate>
            <div class="well">
                ...
                <div class="form-group">
                    ...
                    <asp:TextBox ID="QuantityTextBox" placeholder="Stock Quantity" onkeypress="return isIntegerKey(event)" autocomplete="off" onDrop="blur();return false;" onpaste="return false" runat="server" CssClass="form-control"></asp:TextBox>
                </div>
                <div class="form-group">
                    <label>
                        Commission
                        <asp:RequiredFieldValidator ID="RequiredFieldValidator8" runat="server" ControlToValidate="Seller_Commission_TextBox" CssClass="EroorStar" ErrorMessage="*" ValidationGroup="A"></asp:RequiredFieldValidator></label>
                    <asp:TextBox ID="Seller_Commission_TextBox" placeholder="Commission" onkeypress="return isNumberKey(event)" autocomplete="off" onDrop="blur();return false;" onpaste="return false" runat="server" CssClass="form-control"></asp:TextBox>
                </div>
                <div class="form-group">
                    <asp:CheckBox ID="CIPCheckBox" Enabled="false" runat="server" Text="Add as CIP product" />
                    <span class="alert-danger help-block">Minimum 500 point needed for add CIP product</span>
                </div>
                <div class="form-group">
                    ...
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModal">Update Product Stock</button>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <div class="form-inline">
                    ...
                    <asp:CheckBox ID="SearchCheckBox" AutoPostBack="true" runat="server" Text="CIP Product"/>
                </div>
                ...
                <asp:GridView ID="ProductGridView" runat="server" AutoGenerateColumns="False" CssClass="mGrid table-hover" DataKeyNames="Product_PointID" DataSourceID="Product_PointSQL" AllowSorting="True">
                    <Columns>
                        <asp:TemplateField HeaderText="S.List">
                            <ItemTemplate>
                        ...
                        <asp:TemplateField HeaderText="Point" SortExpression="Product_Point">
                            <EditItemTemplate>
                                <asp:TextBox ID="U_point_TextBox" onkeypress="return isNumberKey(event)" autocomplete="off" onDrop="blur();return false;" onpaste="return false" runat="server" CssClass="form-control" Text='<%# Bind("Product_Point") %>'></asp:TextBox>
                            </EditItemTemplate>
                            <ItemTemplate>
                                <asp:Label ID="U_point_Label" runat="server" Text='<%# Bind("Product_Point") %>'></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        ...
                        <asp:TemplateField HeaderText="Commission" SortExpression="Seller_Commission">
                            <EditItemTemplate>
                                <asp:TextBox ID="comsTextBox" onkeypress="return isNumberKey(event)" autocomplete="off" onDrop="blur();return false;" onpaste="return false" runat="server" CssClass="form-control" Text='<%# Bind("Seller_Commission") %>'></asp:TextBox>
                            </EditItemTemplate>
                            <ItemTemplate>
                                <asp:Label ID="comsLabel" runat="server" Text='<%# Bind("Seller_Commission") %>'></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        ...
                        <asp:TemplateField HeaderText="CIP">
                            <ItemTemplate>
                                <asp:CheckBox ID="Update_cip_CheckBox" AutoPostBack="true" Checked='<%#Eval("IsCIP") %>' runat="server" Text=" " OnCheckedChanged="UpdateCIPCheckBox_CheckedChanged"/>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                    <EmptyDataTemplate>
...
BEGIN
INSERT INTO Product_Point_Code(Product_Name, Product_Code, Product_Point, Product_Price,Stock_Quantity,Seller_Commission,IsCIP) VALUES (@Product_Name, @Product_Code, @Product_Point, @Product_Price,@Stock_Quantity,@Seller_Commission,@IsCIP)
END"
                    SelectCommand="SELECT * FROM [Product_Point_Code] where (IsCIP = @IsCIP OR  @IsCIP = 0) ORDER BY Product_Code" UpdateCommand="IF NOT EXISTS (SELECT * FROM  Product_Point_Code WHERE  (Product_Code = @Product_Code) and  [Product_PointID] &lt;&gt; @Product_PointID)
BEGIN
...
END"
                    ...
                    <InsertParameters>
                        ...
                        <asp:ControlParameter ControlID="Seller_Commission_TextBox" Name="Seller_Commission" PropertyName="Text" />
                        <asp:ControlParameter ControlID="CIPCheckBox" Name="IsCIP" PropertyName="Checked" />
                    </InsertParameters>
                    <SelectParameters>
                        <asp:ControlParameter ControlID="SearchCheckBox" Name="IsCIP" PropertyName="Checked" />
                    </SelectParameters>
                    <UpdateParameters>
                ...
                <asp:SqlDataSource ID="CIPupdateSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="SELECT * FROM [Product_Point_Code]" UpdateCommand="UPDATE [Product_Point_Code] SET IsCIP = @IsCIP WHERE [Product_PointID] = @Product_PointID">
                    <UpdateParameters>
                        <asp:Parameter Name="IsCIP" />
                        <asp:Parameter Name="Product_PointID" Type="Int32" />
                    </UpdateParameters>
                </asp:SqlDataSource>
            </div>
    ...
    <script>
        ...
        function isIntegerKey(evt) { var charCode = (evt.which) ? evt.which : evt.keyCode; return (charCode > 31 && (charCode < 48 || charCode > 57) ? false : true); }
        ...
        $(function () {
            ...
            //Short list CB
            $('[id*=SelectCheckBox]').each(function () {
                $(this).is(":checked") ? ($("td", $(this).closest("tr")).addClass("selected")) : ($("td", $(this).closest("tr")).removeClass("selected"));
            });

            //CIP Check
            $('[id*=Product_PointTextBox]').keyup(function () {
                if (parseInt($(this).val()) >= 500) {
                    $("[id*=CIPCheckBox]").removeAttr("disabled");
                }
                else {
                    $("[id*=CIPCheckBox]").attr("disabled", true).prop('checked', false);
                }
            });          
        });
        ...
        Sys.WebForms.PageRequestManager.getInstance().add_endRequest(function (a, b) {
            ...
            if (parseInt($('[id*=Product_PointTextBox]').val()) >= 500) {
                $("[id*=CIPCheckBox]").removeAttr("disabled");
            }
            else {
                $("[id*=CIPCheckBox]").attr("disabled", true).prop('checked', false);
            ...
            $('[id*=U_point_Label]').each(function () {
                if (parseInt($(this).text()) >= 500) {
                    $("[id*=Update_cip_CheckBox]", $(this).closest("tr")).removeAttr("disabled");
                }
                else {
                    $("[id*=Update_cip_CheckBox]", $(this).closest("tr")).attr("disabled", true);
                }
            });
        });
    </script>
