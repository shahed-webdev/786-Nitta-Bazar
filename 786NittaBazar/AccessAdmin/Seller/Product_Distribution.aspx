<%@ Page Title="Distribution Product" Language="C#" MasterPageFile="~/Basic.Master" AutoEventWireup="true" CodeBehind="Product_Distribution.aspx.cs" Inherits="NittaBazar.AccessAdmin.Seller.Product_Distribution" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="body" runat="server">
    ...
    <asp:SqlDataSource ID="Product_DistributionSQL" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" InsertCommand="INSERT INTO Product_Distribution (SellerID, Admin_RegistrationID, Product_Total_Amount, Product_Total_Point, Distribution_SN, Commission_Amount, Is_Confirmed, Is_Delivered, Delivery_Date, Confirm_Date, Delivered_RegistrationID, Confirmed_RegistrationID) VALUES (@SellerID,@Admin_RegistrationID,@Product_Total_Amount,@Product_Total_Point, dbo.Distribution_SerialNumber(),@Commission_Amount, 1, 1, GETDATE(), GETDATE(),@Admin_RegistrationID,@Admin_RegistrationID)SELECT @Product_DistributionID = Scope_identity()"
        SelectCommand="SELECT * FROM [Product_Distribution]" OnInserted="Product_DistributionSQL_Inserted">
        <InsertParameters>
...
ELSE
BEGIN
UPDATE Seller_Product SET SellerProduct_Stock =SellerProduct_Stock + @SellerProduct_Stock
WHERE        (SellerID = @SellerID) AND (Product_PointID = @Product_PointID)
END">
        <InsertParameters>
            <asp:ControlParameter ControlID="SellerID_HF" Name="SellerID" PropertyName="Value" />
    ...
    <script>
        ...
        $(function () {
            $("[id*=Sell_Button]").click(function () {
                if (localStorage.DistributorCart) {
                    $("[id*=JsonData]").val(JSON.stringify(DistributorCart));
                }
            });

            if (localStorage.DistributorCart) {
                DistributorCart = JSON.parse(localStorage.DistributorCart);
                showCart();
            }
        });
        ...
        function addToCart() {
            ...
            if (Code != '' && Quantity != '' && ProductID != '' && SellerID != '') {
                ...
                for (var i in DistributorCart) {
                    if (DistributorCart[i].ProductID == ProductID) { alert("This Product already added"); return; }
                }
        ...
        function saveCart() {
            if (window.localStorage) {
                localStorage.DistributorCart = JSON.stringify(DistributorCart);
            }
        }
        ...
        $(document).on("click", ".ItemDelete", function () {
            ...
            DistributorCart.splice(index, 1);
            showCart();
            saveCart();
        ...
        $('[id*=DistributorUserNameTextBox]').typeahead({
            ...
            source: function (request, result) {
                $.ajax({
                    url: "Product_Distribution.aspx/GetCustomers",
            ...
            updater: function (item) {
                ...
                if (map[item].IsCIP == 'True') {
                    $("#Seller_type").text("CIP");
                } else {
                    $("#Seller_type").text("General");
                }
        ...
        $('[id*=ProductCodeTextBox]').typeahead({
            ...
            source: function (request, result) {
                $.ajax({
                    url: "Product_Distribution.aspx/GetProduct",
                    data: JSON.stringify({ 'prefix': request, 'SellerID': $("[id$=SellerID_HF]").val() }),
                    dataType: "json",
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    success: function (response) {
            ...
            updater: function (item) {
                ...
                $("#Current_Stook_Label").text(map[item].Net_Quantity);
