<%@ Page Title="Product Short List" Language="C#" MasterPageFile="~/Basic.Master" AutoEventWireup="true" CodeBehind="Product_Short_List.aspx.cs" Inherits="NittaBazar.AccessAdmin.Product_Short_List" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <style>
        .stockOut { background-color: #f68787; }
         .CIP { background-color:#5dd31e}
    </style>
</asp:Content>
                ...
                <asp:GridView ID="ProductGridView" runat="server" AutoGenerateColumns="False" CssClass="mGrid" DataKeyNames="Product_PointID" DataSourceID="Product_PointSQL" AllowSorting="True">
                    <Columns>
                        <asp:TemplateField HeaderText="Product Name" SortExpression="Product_Name">
                            <EditItemTemplate>
                                <asp:TextBox ID="U_ProductTextBox" CssClass="form-control" runat="server" Text='<%# Bind("Product_Name") %>'></asp:TextBox>
                            ...
                            <ItemTemplate>
                                <%# Eval("Product_Name") %>
                                <input class="IsCIPHidden" type="hidden" value='<%#Eval("IsCIP") %>' />
                            </ItemTemplate>
...
END"
    ...
    <script>
        $(function () {
            ...
            $('.mGrid .IsCIPHidden').each(function () {
                var val = $(this).val();
                if (val == "True") {
                    $(this).closest("tr").addClass("CIP");
                }
            });
