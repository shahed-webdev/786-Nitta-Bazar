...
{
    public partial class Load_Balance : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            SMS_Option IsSMS = new SMS_Option();
            SellerTextBox.Visible = IsSMS.Is_load;
        ...
        [WebMethod]
        public static string Get_Seller(string prefix)
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["DBConnectionString"].ConnectionString))
            {
                ...
                {
                    cmd.CommandText = "SELECT top(2) Registration.UserName, Registration.Name, Registration.Phone, Seller.SellerID,Seller.Load_Balance FROM Registration INNER JOIN Seller ON Registration.RegistrationID = Seller.SellerRegistrationID WHERE Registration.UserName LIKE @UserName + '%'";
                ...
                if (SellerID_HF.Value != "")
                {
                    ...
                    if (Send_Balance <= Available_Balance && Send_Balance != 0)
                    {
                        ...
                        if (SMSBalance >= TotalSMS)
                        {
                            ...
                            {
                                Guid? SMS_Send_ID = SMS.SMS_Send(Phone, Msg, "Load Balance");
                                if (SMS_Send_ID != null)
                                {
